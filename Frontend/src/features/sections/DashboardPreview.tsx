import { BarChart3, LineChart, LockKeyhole, Sparkles } from 'lucide-react';
...
const previewInsights = [
  { icon: Sparkles, text: 'AI suggests line chart for monthly trend' },
  { icon: LineChart, text: '3 anomalies found in product segment' },
  { icon: LockKeyhole, text: 'Encrypted connection to data warehouse' },
  { icon: BarChart3, text: 'Auto dashboard created in 8 seconds' },
];
      ...
      <div className="rounded-lg border border-outline-variant bg-surface-container-lowest p-4 shadow-sm">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-outline-variant bg-surface-container-low px-3 py-2 font-mono text-label-caps text-on-surface-variant">
            <span className="size-2 rounded-full bg-cyan-action" />
            Synced
          </div>
        </div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1.45fr_0.95fr]">
          <div className="rounded-lg border border-outline-variant bg-surface p-4">
            ...
            <svg viewBox="0 0 320 100" className="h-40 w-full overflow-visible" role="img" aria-label="Line chart showing growth">
              <polyline
                points={linePoints}
                fill="none"
                stroke="#06b6d4"
